...
widget test scripts in Dart to simulate user interactions on the UI. However,
...
\begin{itemize}
  \item Parses Flutter front-end (\texttt{.dart}) files to extract widget
        metadata;
  \item Uses an LLM (Google Gemini) to synthesize realistic test data;
...
\texttt{WidgetTester} API and a rich set of Finders (\texttt{find.byKey},
...
Rattasarn \cite{b2} proposed a method for generating Robot Framework test
scripts from web front-end files by extracting HTML elements and input
constraints from a database schema. Pantakan \cite{b3} extended this concept
by importing XSD schemas and applying Boundary Value Analysis to produce test
...
The tool statically analyses every \texttt{.dart} file under the
...
The extracted metadata is serialised into a \texttt{manifest.json} file
...
the CO-STEP framework \cite{b4}, which organises the prompt into five
...
\textit{format} (enforces a JSON output schema).
...
front end is a desktop GUI application that guides the user through three
...
paths despite the reduced test-case count.
...
\bibitem{b1}
Google LLC, ``Flutter -- Build apps for any screen,''
\url{https://flutter.dev}, 2024.

\bibitem{b2}
R. Ekakrachawakitti, ``Generating Robot Framework test scripts for web
applications based on database conditions,'' M.S. thesis, Dept. Comput.
Eng., Chulalongkorn Univ., Bangkok, Thailand, 2022.

\bibitem{b3}
P. Srivichayanun, ``Generating Robot Framework test scripts for web
applications from URL and XSD file import,'' M.S. thesis, Dept. Comput.
Eng., Chulalongkorn Univ., Bangkok, Thailand, 2023.

\bibitem{b4}
T. Pham, ``Improving LLM response quality through structured prompt
engineering,'' in \textit{Proc. Int. Conf. Artif. Intell. Appl.}, 2024,
pp. 112--118.

\bibitem{b5}
Google LLC, ``Gemini API documentation,''
\url{https://ai.google.dev}, 2024.

\bibitem{b6}
Flutter Team, ``Testing Flutter apps,''
\url{https://docs.flutter.dev/testing}, 2024.

\bibitem{b7}
D. R. Kuhn, R. N. Kacker, and Y. Lei, ``Introduction to Combinatorial
Testing,'' CRC Press, 2013.

\bibitem{b8}
Microsoft, ``PICT -- Pairwise Independent Combinatorial Testing,''
\url{https://github.com/microsoft/pict}, 2024.

\bibitem{b9}
S. Tiobe\'{e}n, T. Sebastian, and J. Bach, ``Lessons learned in combinatorial
testing,'' \textit{IEEE Softw.}, vol. 21, no. 6, pp. 40--45, Nov./Dec.
2004.
