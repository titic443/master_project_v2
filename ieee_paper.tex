...
\textit{condition file} (a JSON file that declares additional validation
rules for widgets whose logic resides in the Cubit layer rather than in
the widget's own \texttt{validator} callback), the tool merges those
extra \texttt{validatorRules} entries into the corresponding widget
metadata. This allows the tool to generate test data that covers
business-logic constraints that are invisible to static analysis of the
front-end file alone. The condition file follows the same
\texttt{byKey} keying scheme as the manifest, and its rules are
validated for correct syntax before merging; an invalid file triggers an
error message and the pipeline halts.
