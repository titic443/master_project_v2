  ...
  group('clinic_search_page.dart flow (integration)', () {
    group('pairwise_invalid_cases', () {
      testWidgets('pairwise_invalid_cases_1', (tester) async {
        ...
        await tester.enterText(find.byKey(const Key('search_01_patient_name_textfield')), 'ส');
        await tester.pump();
      ...
      testWidgets('pairwise_invalid_cases_2', (tester) async {
        ...
        // Check if any expected element exists (OR logic)
        final expected = [
          find.text('กรุณายืนยันว่ามีสิทธิ์เข้าค้นหา'),
          find.byKey(const Key('search_01_expected_fail')),
        ];
        expect(expected.any((f) => f.evaluate().isNotEmpty), isTrue,
            reason: 'Expected at least one of the elements to exist');
    ...
    group('edge_cases', () {
      ...
      testWidgets('edge_cases_boundary_at_max_length', (tester) async {
        ...
        await tester.enterText(find.byKey(const Key('search_01_patient_name_textfield')), 'พัทธรพรศรีสมบุญ บุญมี สุกัญญา วรรณพร ชัยนรินทร์');
        await tester.pump();
        await tester.ensureVisible(find.byKey(const Key('search_05_insurance_checkbox')));
        await tester.tap(find.byKey(const Key('search_05_insurance_checkbox')));
        await tester.pump();
